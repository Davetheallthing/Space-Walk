...
namespace AC
{
	...
	public class MenuInventoryBox : MenuElement
	{
			...
			if (Application.isPlaying)
			{
				if (uiSlots != null && uiSlots.Length > _slot)
				{
					...
					if (displayType == ConversationDisplayType.IconOnly)
					{
						...
						{
							if (KickStarter.settingsManager.hideSelectedFromMenu && items [_slot+offset] == KickStarter.runtimeInventory.selectedItem)
							{
		...
		public override void Display (GUIStyle _style, int _slot, float zoom, bool isActive)
		{
			...
			if (items.Count > 0 && items.Count > (_slot+offset) && items [_slot+offset] != null)
			{
				if (Application.isPlaying && KickStarter.settingsManager.hideSelectedFromMenu && items [_slot+offset] != null && items [_slot+offset] == KickStarter.runtimeInventory.selectedItem)
				{
					return;
				}
